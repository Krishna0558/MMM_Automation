...
# Save all models as an RDS file
# Suppose OutputModels is a list of models
# Extract a summary dataframe from each model
# model_summaries <- lapply(OutputModels, function(model) {
#   data.frame(
#     ModelID = model$model_id,
#     SomeMetric = model$some_metric,   # replace with actual model fields
#     Param1 = model$param1,
#     Param2 = model$param2
#   )
# })
...
# saveRDS(OutputModels, file = file.path(output_dir, "all_models.rds"))
# library(reticulate)
# use_python("C:/Users/MM3815/Downloads/data_cleaning_app/.venv/Scripts/python.exe", required = TRUE)
# # # Path to your Python if needed
# #
# # # Convert the R object to Python and save as pickle
# py <- import_builtins()
# pickle <- import("pickle")
# #
# # # Open a connection to write the pickle file
# pickle_file <- file.path(output_dir, "all_models.pkl")
# f <- py$open(pickle_file, "wb")
# pickle$dump(OutputModels, f)
# f$close()

# Save summaries of all models — no model ID needed
# write.csv(OutputModels$resultHypParam, file = file.path(output_dir, "all_model_summaries.csv"), row.names = FALSE)
...
# Save Robyn object (for debugging)
# Robyn::robyn_write(
