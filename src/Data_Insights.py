...
def handle_zero_missing(df, target_var):
    ...
    with st.expander("🔍 Zero/Missing Value Analysis", expanded=True):
        ...
        high_missing_cols = missing_or_zero_pct[missing_or_zero_pct > 25].index.tolist()
        ...
        #     # Create a container for imputation details
        #     imputation_details = st.container()
        #
        #     # We'll process all problematic columns, not just top 5
        #     for col in problem_cols:
        #         if col not in numeric_cols:
        #             continue
        #
        #         # Find strongly correlated numeric columns (≥0.8)
        #         corr_matrix = df[numeric_cols].corr()  # Use absolute correlation
        #         potential_helpers = corr_matrix[col].drop(col, errors='ignore')
        #         strong_helpers = potential_helpers[potential_helpers >= 0.7].sort_values(ascending=False)
        #
        #         # Show imputation details in the container
        ...
        #                 valid_mask = mask & df[helper].notnull() & (df[helper] != 0)
        #
        #                 # Impute using median ratio between helper and target column
        #                 ratio = (df[col] / df[helper]).median()
        #                 st.write(f"Imputation ratio (median of {col}/{helper}): {ratio:.2f}")
        #
        #                 df.loc[valid_mask, col] = df.loc[valid_mask, helper] * ratio
        #
        #                 # For remaining missing, use column median
        #                 remaining_missing = (df[col].isnull() | (df[col] == 0)).sum()
        #                 if remaining_missing > 0:
        #                     col_median = df[col].median()
        #                     df.loc[mask & ~valid_mask, col] = col_median
        #                     st.warning(f"Used median ({col_median:.2f}) for {remaining_missing} remaining values")
        #             else:
        #                 st.error(f"No strongly correlated variables (r ≥ 0.8) found for '{col}'")
        #                 # col_median = df[col].median()
        #                 # df[col] = df[col].fillna(col_median).replace(0, col_median)
        #                 # st.info(f"Used column median ({col_median:.2f}) as fallback")
        #
        #             st.markdown("---")
...
def time_lag_analysis(df, target_var, reach_var, time_base):
    with st.expander("📈 Sequential Peak Lag Analysis", expanded=True):
        ...
        df['conversion_peak'] = df[target_var].rolling(7, center=True).max() == df[target_var]
...
def conversion_peak_analysis(df, target_var, spend_var, revenue_var):
    ...
    with st.expander("📊 Conversion Peak Analysis", expanded=True):
        ...
        df['conversion_peak'] = (
            (df[target_var].rolling(7, center=True).max() == df[target_var]) &
...
#     with st.expander("📊 Impact of Frequency Capping Analysis", expanded=True):
#         # Sort data
#         df = df.sort_values('Date').reset_index(drop=True).copy()
#
#         # Mark high reach weeks (top 25%)
#         reach_threshold = df[reach_var].quantile(0.75)
#         high_reach_df = df[df[reach_var] >= reach_threshold]
#
#         if high_reach_df.empty:
#             st.warning("No high-reach periods detected.")
#             return
#
#         # Correlation analysis
#         corr = high_reach_df[[frequency_var, target_var]].corr().iloc[0, 1]
#
#         st.subheader("📈 Frequency vs Conversion (High Reach Weeks)")
#         fig, ax = plt.subplots(figsize=(10, 6))
#         ax.scatter(high_reach_df[frequency_var], high_reach_df[target_var], color='teal', alpha=0.7)
#         ax.set_xlabel('Frequency')
#         ax.set_ylabel('Conversions')
#         ax.set_title('Impact of Frequency on Conversions during High Reach Weeks')
#
#         # Optional: Fit a simple regression line
#         z = np.polyfit(high_reach_df[frequency_var], high_reach_df[target_var], 1)
#         p = np.poly1d(z)
#         ax.plot(high_reach_df[frequency_var], p(high_reach_df[frequency_var]), "r--", label="Trend line")
#
#         # Add legends
#         ax.legend(["Trend line", "High Reach Observations"])
#
#         st.pyplot(fig)
#
#         # Display Summary
...
#         col1.metric("High Reach Threshold", f"{reach_threshold:,.0f}")
#         col2.metric("Frequency-Conversion Correlation", f"{corr:.2f}")
#
#         if corr > 0.2:
#             st.success("Higher frequency tends to improve conversions during high-reach weeks.")
#         elif corr < -0.2:
#             st.error("Higher frequency might be causing saturation (hurting conversions) during high-reach weeks.")
#         else:
#             st.info("No strong relationship between frequency and conversions detected.")
